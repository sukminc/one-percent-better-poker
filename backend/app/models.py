...
class Tournament(Base):
    ...
    # USD P&L after buy-in
    raw_file = Column(Text, nullable=True)
    # raw hand history text
    session_context = Column(JSON, nullable=True)
    # opponent pattern aggregates
    created_at = Column(DateTime, default=datetime.utcnow)

    hands = relationship(
        "Hand", back_populates="tournament", cascade="all, delete-orphan"
    )
...
class Hand(Base):
    ...
    big_blind = Column(Float, nullable=True)            # BB chip value
    ...
    opponent_limped = Column(Boolean, default=False)    # any opponent limped
    facing_limp = Column(Boolean, default=False)        # hero faced a limp
    hero_iso_raised = Column(Boolean, default=False)  # hero iso-raised vs limp
    hero_3bet = Column(Boolean, default=False)  # hero 3-bet preflop
    facing_open_size_bb = Column(Float, nullable=True)  # open raise size (BBs)

    # ── Postflop ─────────────────────────────────────────────────────────────────
    flop_action = Column(String, nullable=True)  # hero flop action
    flop_bet_pct = Column(Float, nullable=True)  # hero bet % of pot (flop)
    facing_oversize_cbet = Column(Boolean, default=False)  # opponent c-bet >75% pot
    ...
    # ── Showdown ────────────────────────────────────────────────────────────────
    hero_cards = Column(String, nullable=True)  # hole cards e.g. As Kd
    showdown_hands = Column(JSON, nullable=True)  # player: cards dict
    showdown_winner = Column(String, nullable=True)  # player who won pot

    # ── Result ───────────────────────────────────────────────────────────────────
    pot_size_bb = Column(Float, nullable=True)  # start pot BBs
    result_bb = Column(Float, nullable=True)  # net +/- BBs
    created_at = Column(DateTime, default=datetime.utcnow)
